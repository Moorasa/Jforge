...
(function(view) {
	...
	var config = {
		labelField: "l\">\x3C\/section>\x3Cscript>alert(1)\x3C\/script>",
		idField: "line sep \x3C\/script>\x3Cscript>evil()\x3C\/script> ${7*7}",
		parentField: "p\" onerror=\"alert(1)",
		iconField: "i\">\x3C\/section>\x3Cscript>x\x3C\/script>"
	};

	// 루트 노드 매핑(commonListTreeView.js 485~486행 root.name/root.iconClass).
	// rootIconClass는 cssToken 화이트리스트 통과 토큰(위반 시 빈 문자열).
	var rootMapping = {
		name: "\x3C\/script>\x3Cscript>alert(1)\x3C\/script> ${9*9}",
		iconClass: "icon"
	};
